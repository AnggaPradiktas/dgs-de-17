...
-- 2.2.2   Locate and click BADGER_DB in the database list.
...
-- 2.2.11  Create a schema in your database, and name it BADGER_SCHEMA.
...
CREATE SCHEMA BADGER_DB.BADGER_SCHEMA;
...
-- 2.5.2   Using SQL, create a warehouse named BADGER_WH2, with the following
--         characteristics.
...
-- 2.5.15  Locate BADGER_WH2 and verify the Status is now Started.
...
-- 2.5.17  Suspend BADGER_WH2:

ALTER WAREHOUSE BADGER_WH2 SUSPEND;
...
-- 2.5.19  Alter BADGER_WH2 and set the WAREHOUSE_SIZE to XSmall:

ALTER WAREHOUSE BADGER_WH2 SET WAREHOUSE_SIZE=XSmall;


-- 2.5.20  Suspend and then drop BADGER_WH2:
...
--         BADGER_WH2 cannot be suspended.
--         Proceed with the statement below to drop the warehouse:

DROP WAREHOUSE BADGER_WH2;


-- 2.5.21  Show warehouses to verify that BADGER_WH is still listed, but
--         BADGER_WH2 is not.

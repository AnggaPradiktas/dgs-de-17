...
-- 3.0.0   Role-Based Access and Design
...
--         confusing and (at worst) completely unmanageable. This lab takes you
--         through the steps needed to identify the role-based requirements and
--         implement them in Snowflake.
--         - Describe a suggested approach to role management.
--         - Summarize the assumed deployment requirements.
--         - Review the remaining scripts.
...
--         lab on a single account without clashing with each other.

-- 3.1.0   Preparation Steps

-- 3.1.1   Identify the virtual warehouses, databases, and schemas required.

-- 3.1.2   Identify the roles and responsibilities (e.g., DBA, ANALYST).

-- 3.1.3   Decide what privileges are needed for each role. For example:

-- 3.1.4   Determine which Databases and Schemas each role will have authority
--         to use. (They will require USAGE privilege on these.)

-- 3.1.5   Decide for each leading role (e.g., DBA) whether they require the
...
-- 3.1.6   Identify which role will go to each user and then verify each
--         person/role has the correct privileges to perform their tasks.

-- 3.1.7   If required, reorganize the roles by dividing responsibilities into
--         other roles.

-- 3.1.8   Create a role hierarchy. (e.g., a power analyst with update access
--         and a sub-set of ANALYST with Read Only access to data).

-- 3.2.0   Creating Databases and Schemas
...
-- 3.3.0   Creating Roles, Users, and Privileges

-- 3.3.1   Use the role SYSADMIN to create database objects (Database,
--         Schema(s), Virtual Warehouses).

-- 3.3.2   Grant the privileges needed by each role to perform their duties
--         including usage on objects.

-- 3.3.3   Create the User(s) for each role.

-- 3.3.4   Where relevant, transfer ownership of each role as needed.

-- 3.3.5   For each role who owns securable objects, switch into the role (use
...
-- 3.6.2   Grant correct privileges to the PROD roles.
--         Note the following items in the subsequent script:

-- 3.6.3   This script grants privileges to the PROD roles:
...
-- 3.6.4   Create the Virtual Warehouse
...
-- 3.6.5   Create the PROD database
--         Note the following items in the subsequent script:

-- 3.6.6   This script creates the PROD database:
...
-- 3.6.7   Grant Privileges to the ANALYST role
--         In this exercie, the fully qualified table name is set using the
--         DATABASE and SCHEMA names rather than setting context with the USE
--         command.
...
-- 3.6.8   Use the following script to GRANT roles and CREATE users.
...
-- Try to log on again as BADGER_prod_dba_bert
...
-- Attempt logging on as BADGER_prod_analyst_ernie


-- 3.6.9   This script transfers ownership of the DEV DBA and DEV roles as
--         appropriate.

-- 3.6.10  Note the following items in the subsequent script:
...
-- 3.6.11  This script grants each of the roles to the currently logged in user.

-- 3.6.12  Change the user ID to your own before executing this script.
...
-- 3.6.13  Use the following script to verify the deployment was successful.

-- 3.6.14  Note the following items in the subsequent script:
--         Role Ownership
...
-- 3.6.15  This section lists an approach to diagnose problems with roles and
--         privileges.

-- 3.6.16  If the object does not exist, this is typically caused by one of the
--         following reasons:

-- 3.6.17  Check your current context

-- 3.6.18  Show the object details (e.g., SHOW databases like %XXX%).

-- 3.6.19  Execute Script in Web UI fails.
...
-- 3.6.20  Click in the Open History link in the results pane to view the
--         failing statement.

-- 3.6.21  Switch between the History tab and the Worksheet tab to diagnose the
--         issue.

-- 3.7.0   Summary of Database Roles and Objects

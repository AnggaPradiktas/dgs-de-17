...
-- 4.0.0   DML and Transaction Model
...
-- 4.1.1   Navigate to the first worksheet and load the script.

-- 4.1.2   Rename one worksheet to SHEET-B.

-- 4.1.3   You will only execute statements in this tab that are surrounded by
--         the SHEET-B comments:

-- SHEET-B --


-- 4.1.4   Navigate to the other worksheet and load the script.

-- 4.1.5   Rename the worksheet to SHEET-A.

-- 4.1.6   You will only execute statements in this tab that are surrounded by
...
-- 4.2.4   Run query and confirm the data is available because the INSERT
...
-- 4.3.1   Set parameter of AUTOCOMMIT to off:
...
-- 4.3.4   Run a query to check for the new records. The two (2) new records are
...
-- 4.3.7   Run the following query in this Worksheet SHEET-B and show it cannot
...
-- 4.3.10  Run the query to select from the table. You should see only three (3)
...
-- 4.3.11  Insert the two (2) rows again in Lab SHEET-A worksheet:
...
-- 4.3.12  Execute a COMMIT statement, and commit the two (2) extra rows:
...
-- 4.4.1   Start a new multi-statement transaction in worksheet Lab SHEET-A:
...
-- 4.4.2   Execute two (2) insert statements in this transaction:
...
-- 4.4.3   End the new multi-statement transaction by running the COMMIT
--         statement:
...
-- 4.4.4   Query the table to see that new rows added by the two (2) INSERT
--         statements above:
...
-- 4.4.5   Insert a new row to the table:
...
-- 4.4.6   Execute a ROLLBACK statement:
...
-- 4.4.7   Confirm that AUTOCOMMIT in current session is still set to FALSE:
...
-- 4.4.8   Query the table. You should see only eight (8) rows because the newly
...
-- 4.5.2   Use the SHOW LOCKS command to show that SELECT does not place any
...
-- 4.5.3   Confirm that AUTOCOMMIT in current session is still set to FALSE:
...
-- 4.5.5   Check that the UPDATE statement succeeds and verify the record has
--         indeed been updated.
--         Check Update
...
-- 4.5.6   Run the SHOW LOCKS command to confirm that the UPDATE statement has
...
-- 4.5.9   While remaining in worksheet Lab SHEET-B, run the following DELETE
...
-- 4.5.15  Check that the DELETE statement is completed now, as shown below,
...
-- 4.6.0   Use SYSTEM$ABORT_TRANSACTION to Release Locks and Terminate
--         Transactions

-- 4.6.1   SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-A
...
-- 4.6.5   Review the transaction id associated with the DELETE statement.

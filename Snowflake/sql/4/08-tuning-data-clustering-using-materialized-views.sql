...
-- 8.0.0   Tuning Data Clustering Using Materialized Views
...
--         views.
--         - Cluster a table using a TIMESTAMP column.
--         - Create tables and checking clustering for attributes.
--         - Cluster a table using a PAGE_ID column.

-- 8.1.0   ­­Cluster a Table Using a Timestamp Column
...
-- 8.1.1   Navigate to Worksheets and create a new worksheet.

-- 8.1.2   Name the worksheet Clustering & MV.

-- 8.1.3   Set the worksheet context as follows:
...
-- 8.2.0   Create a Table and Check Clustering for an Attribute

-- 8.2.1   Create a table by cloning a table in the TRAINING_DB database.
...
-- 8.2.2   Check the clustering quality of the attribute, CREATE_MS. It is
--         expected to be good.
...
-- 8.2.3   Run the following query containing a search filter using the column
...
-- 8.2.4   Check the query profile of above query.

-- 8.2.5   Notice that with the TableScan[2] operator the partition pruning is
--         very good:
--         Partitions scanned: 1
--         Partition total: 20,683
--         Query Profile 2

-- 8.2.6   Check for clustering on the other column, PAGE_ID. It is expected to
--         be poor.

SELECT SYSTEM$CLUSTERING_INFORMATION( 'WEBLOG' , '(PAGE_ID)' );


-- 8.2.7   Note the result from running above query:
...
-- 8.2.8   Note that query performance is expected to be sub­optimal using the
--         filter with PAGE_ID:
...
-- 8.2.9   Check the query profile of the above query. Notice that with the
--         TableScan[2] operator the partition pruning is quite poor. It is
--         scanning all partitions.
--         Partition scanned: 20683
--         Partition total: 20683
--         Full Table Scan

-- 8.3.0   Cluster a Table Using a PAGE_ID Column
...
-- 8.3.1   Creating the materialized view with Snowflake allows us to specify
...
-- 8.3.2   After completing the materialized view build, validate optimal data
--         distribution by retrieving the clustering information.

-- 8.3.3   Run the following query:

SELECT SYSTEM$CLUSTERING_INFORMATION
( 'MV_TIME_TO_LOAD' , '(PAGE_ID)' );


-- 8.3.4   Review the query results:
...
-- 8.3.5   Check the runtime for the same query against the Materialized View.
--         Testing shows the response time is similar to the lookup by CREATE_MS
--         column against the base table.

-- 8.3.6   Run the following query:
...
-- 8.3.7   Check the query profile. Notice that with the TableScan[1] operator
--         the partition pruning is very effective:
--         Partition scanned: 1
--         Partition total: 6381
--         Materialized View Profile

-- 8.3.8   Use the SHOW command to display information about the materialized
--         view created:
